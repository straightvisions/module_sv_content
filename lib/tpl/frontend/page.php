<?php
	while ( have_posts() ) {
		...
		if ( $this->show_right_sidebar() && empty($this->get_module( 'sv_sidebar' )->load( $this->get_prefix('page_right' ))) === false) {
			$post_classes .= $this->get_prefix( 'has_sidebar_right' );
		}
		
		if ( $this->show_bottom_sidebar() && empty($this->get_module( 'sv_sidebar' )->load( $this->get_prefix('page_bottom' ))) === false) {
